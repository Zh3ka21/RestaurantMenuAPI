...
def create_menu_endpoint(
    menu: MenuCreate, db: Session = Depends(get_db),
) -> MenuResponse:
    """Menu create view."""
...
def get_today_menu_endpoint(db: Session = Depends(get_db),
                            current_user: Employee = Depends(get_current_user)) -> MenuResponse:
